...
export class GoJSWrapper extends React.Component<WrapperProps, WrapperState> {
  /**
  ...
  private initDiagram(): go.Diagram {
    ...
    diagram.linkTemplate =
      $(go.Link,
        ...
        $(go.Shape),
        $(go.Shape, { toArrow: 'Standard', fromArrow: 'Standard' })
      );
  ...
  private initPalette(): go.Palette {
    ...
    // palette.nodeTemplateMap.add("", $(go.Node, 'Auto', $(go.Shape, 'Circle', {fill: 'red'})));

    palette.nodeTemplateMap.add("", $(go.Node, 'Auto',  // the Shape will go around the TextBlock
      new go.Binding('location', 'loc', go.Point.parse).makeTwoWay(go.Point.stringify),
      $(go.Shape, 'Circle',
        {
          name: 'SHAPE', fill: 'lightgray', strokeWidth: 0,
      ...
      <div className='gojs-wrapper-div'>
        <ReactPalette
          ...
          nodeDataArray={
            [
              { key: 0, text: 'Alpha' },
              { key: 1, text: 'Beta', color: 'green' },
              { key: 2, text: 'Gamma', color: 'blue' },
              { key: 3, text: 'Delta', color: 'beige' }
            ]
          }
        />
        <ReactDiagram
